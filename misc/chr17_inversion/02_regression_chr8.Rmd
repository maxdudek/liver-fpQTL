...
```{r}
location <- c(6300001, 12800000)

variant_info_chr8_inversion <- variant_info %>%
  filter(variant_chrom == "chr8", variant_pos > location[1], variant_pos < location[2])

genotype_matrix_chr8_inversion <- genotype_matrix[variant_info_chr8_inversion$variant_id, ]
fpscore_matrix_chr8_inversion <- fpscore_matrix[variant_info_chr8_inversion$variant_id, ]

fpscore_cov_regression_chr8_inversion <- fpscore_cov_regression %>%
  filter(variant_chrom == "chr8", variant_pos > location[1], variant_pos < location[2])

variant_info_chr8_inversion %>% saveRDS("data/variant_info_chr8_inversion.Rds")
fpscore_matrix_chr8_inversion %>% saveRDS("data/fpscore_matrix_chr8_inversion.Rds")
genotype_matrix_chr8_inversion %>% saveRDS("data/genotype_matrix_chr8_inversion.Rds")
fpscore_cov_regression_chr8_inversion %>% saveRDS("data/fpscore_cov_regression_chr8_inversion.Rds")
```

# Load subsetted matrices
```{r}
variant_info_chr8_inversion <- readRDS("data/variant_info_chr8_inversion.Rds")
fpscore_matrix_chr8_inversion <- readRDS("data/fpscore_matrix_chr8_inversion.Rds")
genotype_matrix_chr8_inversion <- readRDS("data/genotype_matrix_chr8_inversion.Rds")
fpscore_cov_regression_chr8_inversion <- readRDS("data/fpscore_cov_regression_chr8_inversion.Rds")
```
...
inversion_genotypes <- read.delim("data/chr8_inversion_genotypes.txt")
...
regression_covariates <- regression_covariates %>%
  ...
  filter(sample_id %in% colnames(fpscore_matrix_chr8_inversion)) %>%
  arrange(match(sample_id, colnames(fpscore_matrix_chr8_inversion)))
```
...
run_regression_on_variant <- function(i, covariates = TRUE) {
  ...
  x <- genotype_matrix_chr8_inversion[i,]
  y <- fpscore_matrix_chr8_inversion[i,]
...
regression_result <- sapply(1:nrow(fpscore_matrix_chr8_inversion), run_regression_on_variant,
                                covariates=TRUE)

t(regression_result) %>%
    as.data.frame() %>%
    mutate(variant_id = rownames(fpscore_matrix_chr8_inversion)) %>%
    cbind(variant_info_chr8_inversion %>% select(-variant_id)) ->
    fpscore_regression_chr8_inversion

# cat("Verifying that variant_ids line up in regression_results:\n")
print(all(fpscore_regression_chr8_inversion$variant_id == variant_info_chr8_inversion$variant_id))

fpscore_regression_chr8_inversion %>% 
  write.table("data/fpscore_regression_chr8_inversion.txt",
              quote = FALSE, row.names = FALSE, sep = "\t")
```

```{r}
fpscore_regression_chr8_inversion <- read.delim("data/fpscore_regression_chr8_inversion.txt")
```

```{r fig.width=6, fig.height=4}
pdf("figures/chr8_inversion_manhattan_plots.pdf", width = 7, height = 4)

fpscore_cov_regression_chr8_inversion %>%
